...
def build_comparison_ui(factors, prefix, group_name):
    n = len(factors)
    matrix = np.ones((n, n))
    answers = []
    ...
    for r in range(n):
        for c in range(r + 1, n):
            ...
            answers.append({
                ...
                "Фактор B": factors[c],
                "Выбор": choice,
                "Оценка Саати": score if choice != "Равны" else 1,
                "Значение A/B": round(val, 6),
            })
            st.write("---")
...
with st.expander("Шаг 1. Сравнение главных критериев (Уровень 1)", expanded=True):
    ...
    matrices["main"], answers_by_group["main"] = build_comparison_ui(MAIN_CRITERIA, "main", "Главные критерии")
...
for idx, main_crit in enumerate(MAIN_CRITERIA):
    with tabs[idx]:
        ...
        matrices[main_crit], answers_by_group[main_crit] = build_comparison_ui(sub_factors, f"sub_{idx}", main_crit)
...
if st.button("Проверить согласованность и рассчитать результат", type="primary"):
    ...
    if main_is_consistent and all(sub_consistencies):
        ...
        # Формирование единого CSV с ответами и весами
        rows = []
        rows.extend(answers_by_group.get("main", []))
        for main_crit in MAIN_CRITERIA:
            rows.extend(answers_by_group.get(main_crit, []))

        for i, main_crit in enumerate(MAIN_CRITERIA):
            rows.append({
                "Тип строки": "Вес_группы",
                "Группа": "Главные критерии",
                "Фактор A": main_crit,
                "Фактор B": "",
                "Выбор": "",
                "Оценка Саати": "",
                "Значение A/B": "",
                "Локальный вес": round(main_weights[i], 6),
                "Глобальный вес": round(main_weights[i], 6),
            })

            for j, sub_factor in enumerate(HIERARCHY[main_crit]):
                local_weight = float(sub_weights[main_crit][j])
                global_weight = float(main_weights[i] * local_weight)
                rows.append({
                    "Тип строки": "Вес_подфактора",
                    "Группа": main_crit,
                    "Фактор A": sub_factor,
                    "Фактор B": "",
                    "Выбор": "",
                    "Оценка Саати": "",
                    "Значение A/B": "",
                    "Локальный вес": round(local_weight, 6),
                    "Глобальный вес": round(global_weight, 6),
                })
        ...
        st.download_button(
            label="Скачать CSV с ответами и весами",
            data=csv_text.encode("utf-8-sig"),
            file_name="saaty_answers_and_weights.csv",
            mime="text/csv",
        )
        st.info("CSV сформирован. Нажмите кнопку выше, чтобы скачать файл.")
    else:
        st.error(
